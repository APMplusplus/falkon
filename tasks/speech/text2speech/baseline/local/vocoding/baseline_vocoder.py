...
def generate(partial_flag = 1):

 model_name = model_dir + '/model_debug_wavenet_pitchmarkconditionedmodel_epoch_421.pth'
 ...
 with torch.no_grad():
    ...
    for i, file in enumerate(val_loader):
       ...
       c = torch.FloatTensor(ccoeffs)
       c =  Variable(c)
       if torch.cuda.is_available():
             c = c.cuda()
       c = c.unsqueeze(0)
       assert len(c.shape) == 3
       x_hat = model.forward_incremental(None, c, 1)
       x_hat = torch.max(x_hat,2)[1]
       x_hat = x_hat.cpu().numpy()
       x_hat = inv_mulaw_quantize(x_hat)
              
       sf.write('wav_predictions/' + file + '_predicted' + '.wav', np.asarray(x_hat), 16000,format='wav',subtype="PCM_16")
                  
       # Return after a single file           
       return            
 ...
 with torch.no_grad():
    ...
    for i, file in enumerate(val_loader):
       ...
       if plot_flag:
              ...
              plt.savefig('plots_debug/plot_step' + str(updates).zfill(4) + '.png')
              plt.close()
...
def train():
  ...
  for i, files in enumerate(train_loader):
    ...
    for file in files:
       ...
       x = wav_quantized[start_sample:end_sample]
       c = ccoeffs[start_frame:end_frame]
       #print("Shapes of x and c: ", x.shape, c.shape)
       x,c = ensure_frameperiod(c, x, frame_period)
       assert len(x) == max_timesteps
       assert len(x) == len(c) * frame_period
       x_batch.append(x)
