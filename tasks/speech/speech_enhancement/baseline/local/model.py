...
class baseline_lstm(baseline_model):
        ...
        def __init__(self):
          ...
          self.encoder_fc = layers.SequenceWise(nn.Linear(39, 32))
          ...
          self.final_fc = nn.Linear(128, 39)

        def forward(self, c):

           x = self.encoder_fc(c)
           if print_flag:
              print("Shape of output from encoder is ", x.shape)
           x, hidden = self.seq_model(x, None)
           x = self.final_fc(x)
           return x
